...
if not anomaly_users.empty:
    for idx, (_, row) in enumerate(anomaly_users.head(20).iterrows()):
        anomaly_type, severity = anomaly_types[idx % len(anomaly_types)]
        anomaly_detail_data.append({
            "user_id": row["user_id"],
            "anomaly_type": anomaly_type,
            "severity": severity,
            "anomaly_score": round(float(row.get("anomaly_score", rng.uniform(-0.5, -0.1))), 3),
            "detected_at": (pd.to_datetime("2025-01-31") - pd.Timedelta(hours=int(rng.integers(1, 72)))).isoformat(),
            "detail": f"비정상 패턴 {idx+1}번째 감지",
        })
