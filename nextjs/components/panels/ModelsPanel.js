...
// CookieRun 샘플 모델 레지스트리 (train_models.py와 일치)
const SAMPLE_MODELS = [
  {
    name: '번역품질예측',
    description: '세계관 번역 품질 평가 모델 (RandomForest)',
    model_type: 'sklearn',
    versions: [
      { version: 1, stage: 'Production', created_at: '2026-01-30' },
    ]
  },
  {
    name: '텍스트분류',
    description: '텍스트 카테고리 분류 모델 (TF-IDF + RandomForest)',
    model_type: 'sklearn',
    versions: [
      { version: 1, stage: 'Production', created_at: '2026-01-30' },
    ]
  ...
  {
    name: '유저세그먼트',
    description: '유저 세그먼트 클러스터링 모델 (KMeans)',
    model_type: 'sklearn',
    versions: [
      { version: 1, stage: 'Production', created_at: '2026-01-30' },
    ]
  ...
  {
    name: '이상탐지',
    description: '이상 행동 유저 탐지 모델 (IsolationForest)',
    model_type: 'sklearn',
    versions: [
      { version: 1, stage: 'Production', created_at: '2026-01-30' },
    ]
  ...
  {
    name: '이탈예측',
    description: '유저 이탈 예측 모델 (XGBoost + SHAP)',
    model_type: 'sklearn',
    versions: [
      { version: 1, stage: 'Production', created_at: '2026-01-30' },
    ]
  ...
  {
    name: '승률예측',
    description: '쿠키 스탯 기반 PvP 승률 예측 모델 (LightGBM)',
    model_type: 'lightgbm',
    versions: [
      { version: 1, stage: 'Production', created_at: '2026-01-30' },
    ]
...
// CookieRun 샘플 실험 데이터 (train_models.py와 일치)
const SAMPLE_EXPERIMENTS = [
  {
    ...
    runs: [
      {
        run_id: 'run_001',
        run_name: 'translation_quality_model',
        status: 'FINISHED',
      ...
      {
        run_id: 'run_004',
        run_name: 'anomaly_detection_model',
        status: 'FINISHED',
      ...
      {
        run_id: 'run_005',
        run_name: 'churn_prediction_model',
        status: 'FINISHED',
      ...
      {
        run_id: 'run_006',
        run_name: 'win_rate_model',
        status: 'FINISHED',
        start_time: Date.now() - 518400000,
        metrics: {
          rmse: 2.85,
          mae: 2.12,
          r2_score: 0.78,
        ...
        params: {
          ...
          n_estimators: 100,
          learning_rate: 0.05,
          features: 'atk,hp,def,skill_dmg,cooldown,crit_rate,crit_dmg',
        }
      },
    ]
  },
    ...
    async function fetchMLflowData() {
      ...
      try {
        ...
        if (expRes?.status === 'SUCCESS' && expRes.data?.length > 0) {
          // CookieRun 관련 실험만 필터링 (한글/영어 모두 지원)
          const cookierunExps = expRes.data.filter(exp => {
            const name = exp.name.toLowerCase();
            return name.includes('cookie') ||
              name.includes('cookierun') ||
              name.includes('ai-platform') ||
              name.includes('번역') ||
              name.includes('세그먼트') ||
              name.includes('이상') ||
              name.includes('이탈') ||
      ...
      try {
        ...
        if (modelsRes?.status === 'SUCCESS' && modelsRes.data?.length > 0) {
          // 모든 모델 사용 (한글 이름: 번역품질예측, 텍스트분류, 유저세그먼트, 이상탐지, 이탈예측, 승률예측)
