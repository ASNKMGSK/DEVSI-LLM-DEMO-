...
import { ArrowUpRight, Sparkles, Zap, Loader2, Cookie, Copy, RefreshCcw, Check } from 'lucide-react';
import { fetchEventSource } from '@microsoft/fetch-event-source';

// CookieRun 테마 버튼 스타일 - 갈색 파스텔
const cookieBtn =
  'w-full rounded-2xl border-2 border-amber-300/50 bg-gradient-to-r from-amber-100 to-amber-50 px-4 py-3 text-sm font-extrabold text-cookie-brown shadow-md transition hover:from-amber-200 hover:to-amber-100 hover:border-amber-400/50 active:translate-y-[1px] disabled:opacity-60 disabled:cursor-not-allowed';

const cookieBtnSecondary =
  'w-full rounded-2xl border-2 border-amber-200/50 bg-amber-50 px-4 py-3 text-sm font-extrabold text-cookie-brown shadow-sm transition hover:bg-amber-100 active:translate-y-[1px] disabled:opacity-60 disabled:cursor-not-allowed';

const cookieBtnInline =
  'rounded-2xl border-2 border-amber-300/50 bg-gradient-to-r from-amber-100 to-amber-50 px-4 py-3 text-sm font-extrabold text-cookie-brown shadow-md transition hover:from-amber-200 hover:to-amber-100 hover:border-amber-400/50 active:translate-y-[1px] disabled:opacity-60 disabled:cursor-not-allowed inline-flex items-center justify-center gap-2 whitespace-nowrap';

const cookieBtnSecondaryInline =
  'rounded-2xl border-2 border-amber-200/50 bg-amber-50 px-4 py-3 text-sm font-extrabold text-cookie-brown shadow-sm transition hover:bg-amber-100 active:translate-y-[1px] disabled:opacity-60 disabled:cursor-not-allowed inline-flex items-center justify-center gap-2 whitespace-nowrap';
...
function TopProgressBar({ active }) {
  ...
  return (
    <div className="mb-3 h-1 w-full overflow-hidden rounded-full bg-cookie-yellow/30">
      <div className="h-full w-1/3 animate-[cookierun_progress_1.2s_ease-in-out_infinite] bg-cookie-orange" />
    </div>
  );
}
...
export default function AgentPanel({
  ...
  useEffect(() => {
    ...
    window.addEventListener('cookierun_send_question', handler);
    return () => window.removeEventListener('cookierun_send_question', handler);
  }, [sendQuestion]);
