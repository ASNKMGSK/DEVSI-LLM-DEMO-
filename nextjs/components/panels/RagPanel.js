...
export default function RagPanel({ auth, apiCall, addLog }) {
  ...
  const [showGraphRagTooltip, setShowGraphRagTooltip] = useState(false);
      ...
      <div className="rounded-3xl border border-cookie-brown/10 bg-white/70 p-5 shadow-sm backdrop-blur">
        ...
        <div className="mt-4 rounded-2xl border border-indigo-200 bg-indigo-50/50 p-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            {/* Reranking */}
            <div className="flex items-center gap-2">
              <Zap size={14} className={status?.reranker_available ? 'text-green-600' : 'text-cookie-brown/50'} />
              <div>
            ...
            <div className="flex items-center gap-2">
              <GitBranch size={14} className={status?.kg_ready ? 'text-green-600' : 'text-cookie-brown/50'} />
              <div>
            ...
            {/* GraphRAG (LLM 기반) */}
            <div className="flex items-center gap-2">
              <Network size={14} className={status?.graphrag_ready ? 'text-green-600' : 'text-cookie-brown/50'} />
              <div>
                <div className="text-xs font-black text-cookie-brown/80">GraphRAG</div>
                <div className="text-[10px] text-cookie-brown/60">
          ...
          {status?.graphrag_available && auth?.user_role === '관리자' && (
            <div
              ...
              onMouseEnter={() => setShowGraphRagTooltip(true)}
              onMouseLeave={() => setShowGraphRagTooltip(false)}
            >
              <button
                disabled={true}
                className="w-full rounded-xl border border-cookie-brown/20 bg-cookie-beige px-3 py-2 text-xs font-black text-cookie-brown/50 cursor-not-allowed flex items-center justify-center gap-2"
                type="button"
              >
                <Network size={14} />
                GraphRAG 빌드 (비활성화됨)
              ...
              >
                <div className="bg-cookie-brown text-white text-xs rounded-xl px-4 py-3 shadow-lg">
                  ...
                  <p className="text-cookie-beige leading-relaxed">
                    GraphRAG 빌드는 LLM API를 사용하여 <span className="text-white font-semibold">비용이 발생</span>합니다.
        ...
        <button
          ...
          disabled={loading}
          className="mt-4 w-full rounded-2xl border border-cookie-brown/20 bg-cookie-beige px-4 py-2 text-sm font-black text-cookie-brown/80 hover:bg-cookie-beige/80 disabled:opacity-50"
          type="button"
        >
      ...
      <div className="rounded-3xl border border-cookie-brown/10 bg-white/70 p-5 shadow-sm backdrop-blur">
        ...
        <div className="space-y-3">
          <div className="rounded-2xl border-2 border-dashed border-cookie-brown/20 bg-cookie-beige/50 p-6 text-center">
            <Upload size={32} className="mx-auto mb-3 text-cookie-brown/50" />
            <input
              type="file"
            ...
            <label
              htmlFor="file-upload"
              className="inline-flex items-center gap-2 rounded-2xl border border-cookie-brown/20 bg-white px-4 py-2 text-sm font-black text-cookie-brown/80 hover:bg-cookie-beige cursor-pointer"
            >
              <Upload size={16} />
              파일 선택
            </label>
            ...
            {selectedFile && (
              <div className="mt-3 text-sm font-semibold text-cookie-brown/70">
          ...
          <button
            ...
            className="w-full rounded-2xl border border-amber-800 bg-gradient-to-r from-amber-700 to-amber-800 px-4 py-3 text-sm font-black text-white shadow-lg hover:from-amber-800 hover:to-amber-900 disabled:opacity-50 disabled:cursor-not-allowed"
            type="button"
          >
            {uploading ? '업로드 중...' : '업로드'}
          </button>
        ...
        {ocrResult && (
          <div className="mt-4 rounded-2xl border border-green-200 bg-green-50 p-4">
            ...
            <div className="rounded-xl border border-green-200 bg-white p-3 max-h-40 overflow-y-auto">
              <pre className="text-xs text-cookie-brown/80 whitespace-pre-wrap font-mono">
        ...
        {files.length === 0 ? (
          ...
          <div className="space-y-2">
            {files.map((file) => (
              <div
                key={file.filename}
                className="flex items-center justify-between gap-3 rounded-2xl border border-cookie-brown/10 bg-white p-3 hover:shadow-sm transition"
              >
