    ...
    try {
      ...
      if (res?.status === 'SUCCESS') {
        ...
        setDrilldownData({
          ...
          avg_level: Math.floor(20 + Math.random() * 50),
          avg_playtime: Math.floor(50 + Math.random() * 200),
          avg_cookies_owned: Math.floor(5 + Math.random() * 30),
          top_activities: ['스테이지 클리어', '가챠', 'PvP'],
          retention_rate: (60 + Math.random() * 35).toFixed(1),
        });
      }
    } catch (e) {
      // 폴백 데이터
            ...
            <div className="rounded-3xl border-2 border-red-200 bg-gradient-to-br from-red-50 to-white p-5 shadow-sm">
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="ml-auto px-2 py-0.5 rounded-full bg-red-500 text-white text-[10px] font-bold">
                  {alerts.length || dashboard?.user_stats?.anomaly_count || 0}
                </span>
              </div>
              <div className="space-y-3">
            ...
            >
              ...
              <div className="mt-3 flex gap-4">
                <div>
                  ...
                  <div className="text-xs opacity-80">총 유저 수</div>
                </div>
                <div>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    <div className="p-3 rounded-xl bg-cookie-cream/50">
                      <div className="text-xs text-cookie-brown/60 mb-1">평균 레벨</div>
                      <div className="text-lg font-black text-cookie-brown">Lv.{drilldownData.avg_level || '-'}</div>
                    </div>
                    <div className="p-3 rounded-xl bg-cookie-cream/50">
                      <div className="text-xs text-cookie-brown/60 mb-1">평균 플레이타임</div>
                      <div className="text-lg font-black text-cookie-brown">{drilldownData.avg_playtime || '-'}h</div>
                    </div>
                    <div className="p-3 rounded-xl bg-cookie-cream/50">
                      <div className="text-xs text-cookie-brown/60 mb-1">보유 쿠키 수</div>
                      <div className="text-lg font-black text-cookie-brown">{drilldownData.avg_cookies_owned || '-'}개</div>
                    </div>
                    <div className="p-3 rounded-xl bg-cookie-cream/50">
